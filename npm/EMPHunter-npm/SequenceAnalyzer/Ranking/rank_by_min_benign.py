...
class ranker():
    ...
    def get_heart(self, batch_name, benign_batch_name, vec_file_name, label_list, benign_limit):
        ...
        with open(os.path.join(CURRENT_DIR, "../../DataShare/BaseData/Benign", benign_batch_name, vec_file_name),
                  "r") as fp:
    ...
    def ranking_top_cluster_weight(self, max_n=5):
        max_n_clust = heapq.nlargest(max_n, self.clust_num_dict.items(), key=lambda d: d[1])
        biggest_list = []
        for i in max_n_clust:
            biggest_list.append(int(i[0]))
    ...
    def ranking_biggest_cluster_weight_with_malicious(self, max_n, malicious_batch_name, malicious_vec_file):
        ...
        for label_num, vec in self.vec_dict.items():
            ...
            for label, heart_vec in self.clust_heart_dict.items():
                if label in biggest_list:
                    cos_dist = spatial.distance.cosine(vec, heart_vec)
                    dist_list.append(cos_dist)
            self.dist_dict[label_num - self.benign_len] = dist_list
        ...
        for label_num, dist_list in self.dist_dict.items():
            ...
            score = score / max_n - malicious_dist_dict[label_num]
            self.rank_dict[label_num] = score

        # # test
        # for label_num, dist_list in self.dist_dict.items():
        #     score = - malicious_dist_dict[label_num]
        #     self.rank_dict[label_num] = score

        return self.rank_dict

    def ranking_nearest_cluster_weight_with_malicious(self, max_n, malicious_batch_name, malicious_vec_file):
        # get malicious dist dict
        with open(os.path.join(CURRENT_DIR, "../../DataShare/BaseData/Malicious/",
                               malicious_batch_name, malicious_vec_file)) as fp:
            malicious_vec_lines = fp.readlines()
        ...
        for label_num, vec in self.vec_dict.items():
            ...
            sorted_dict = sorted(dist_dic.items(), key=lambda x: x[1], reverse=True)
            # 只取字典的前max_n个
            near_cluster_dist_dict = dict(sorted_dict[:max_n])
